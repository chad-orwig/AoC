...
const firstMap = /(\w+)\s\((\d+)\)(\s->.*$)?/;
const regex2 = /\s->\s|,\s/;
const inputArray = input.split('\n');

function determineSupports(result) {
    return result.split(regex2);
}

const labeledInput = inputArray.map(line => {
   const results = firstMap.exec(line);
   return {
       name : results[1],
       weight : Number(results[2]),
       supports : results[3] ? determineSupports(results[3]) : undefined
   };
});
