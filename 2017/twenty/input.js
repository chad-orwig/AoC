const input = 'p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>\n' +
...
const regex = /p=<([\d,\-]+)>, v=<([\d,\-]+)>, a=<([\d,\-]+)>/;
const lines = input.split('\n');
const arrs = lines.map(line => regex.exec(line));
const _ = require('lodash');
const objects = _.map(arrs, (arr, index) => {
    const [match, pString, vString, aString] = arr;
    const a = aString.split(',').map(Number);
    const p = pString.split(',').map(Number);
    const v = vString.split(',').map(Number);
    return { p,v,a, index };
});
