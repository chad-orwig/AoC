...
impl <'a> AntinodeIter<'a> {
    pub fn new(start: Loc<usize>, pair: Loc<usize>, map: &'a Vec<Vec<Space>>) -> Self {
...
fn all_antinodes<'a>(left: Loc<usize>, right: Loc<usize>, map: &'a Vec<Vec<Space>>) -> impl Iterator<Item = Loc<usize>> + use<'a> {
    return AntinodeIter::new(left, right, map)
        .chain(AntinodeIter::new(right, left, map))
}

fn first_antinode(loc_1: Loc<usize>, loc_2: Loc<usize>) -> Vec<Option<Loc<usize>>>{
...
fn all_first_antinodes(frequencies: &Vec<Loc<usize>>) -> impl Iterator<Item=Option<Loc<usize>>> {
...
fn all_all_antinodes<'a>(frequencies: &Vec<Loc<usize>>, map: &'a Vec<Vec<Space>>) -> impl Iterator<Item=Loc<usize>> + use<'a> {
    return iproduct!(frequencies.clone(), frequencies.clone())
        .filter(|(left, right)| left != right)
...
fn main() {
    ...
    let frequency_locs = map.iter_loc()
        .fold(HashMap::new(), |mut h_map: HashMap<Space, Vec<Loc<usize>>>, (loc, space): (Loc<usize>, &Space) | match space {
            Space::Empty => h_map,
            Space::Frequency(_) => {
