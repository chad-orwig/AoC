...
Game 100: 1 blue, 13 green, 14 red; 11 green, 11 blue, 7 red; 2 red, 1 blue, 2 green; 10 blue, 15 red`
  ...
  /**
   * @typedef Colors
   * @property {number|undefined} red
   * @property {number|undefined} green
   * @property {number|undefined} blue
   * 
   * @classdesc
   * @class
   * 
...
export function Game(s) {
  const split = s.split(': ');
  /** @type {number} */
  this.id = Number(split[0].slice(5));
  /** @type {Draw[]} */
  this.draws = split[1].split('; ')
    .map(ds => new Draw(ds));
}
 ...
 * @returns {boolean}
 */
Game.prototype.isPossible = function(limits) {
  return !this.draws.find(d => d.colors.red > limits.red) &&
  !this.draws.find(d => d.colors.blue > limits.blue) &&
  !this.draws.find(d => d.colors.green > limits.green)
}

/**
 * @returns {Colors}
 */
Game.prototype.fewestColors = function() {
  const red = Math.max(...this.draws.map(d => d.colors.red).filter(n => n !== undefined));
  const green = Math.max(...this.draws.map(d => d.colors.green).filter(n => n !== undefined));
  const blue = Math.max(...this.draws.map(d => d.colors.blue).filter(n => n !== undefined));
  return {
    red,
    green,
    blue,
...
export const test = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
