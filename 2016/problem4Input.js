...
module.exports = lines.map(decode);

function decode(line) {
    const finalDash = line.lastIndexOf('-');
    const encryptedName = line.substring(0, finalDash);
    const encryptedNameWithoutDashes = encryptedName.replace(/-/g, '');
    const end = line.substring(finalDash + 1);
    const sector = Number(/\d+/.exec(end)[0]);
    const checksum = /[a-z]+/.exec(end)[0];

    return {
        encryptedName,
        encryptedNameWithoutDashes,
        sector,
        checksum
    }
